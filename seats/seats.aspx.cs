...
public partial class seats_seats : System.Web.UI.Page
{
    public string price;
    public string sid;
    public string seats;
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Session["bid"] = sid.ToString();
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["ysd"].ToString());
        con.Open();
        String rq = "select Id,BusTo,BusFrom,dep_time,arr_time,km,rate from BusRoute where BusTo='"+ Session["destination"].ToString() +"' and BusFrom='"+ Session["source"].ToString() +"' ";
        ...
        if (sdr.Read())
        {
            ...
            Session["bustime"] = sdr["dep_time"].ToString();
        }
        sdr.Close();
    }
    public void removeseat()
    {
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["ysd"].ToString());
        con.Open();
        String rq = "select seats,journeydate from booking where journeydate='" + Session["journeydate"].ToString() + "' and source='" + Session["source"].ToString() + "' and destination='" + Session["destination"].ToString() + "'";
        SqlCommand cmd = new SqlCommand(rq, con);
        SqlDataReader sdr = cmd.ExecuteReader();
        if (sdr.Read())
        {
    ...
    protected void pay_Click(object sender, EventArgs e)
    {
        ...
        Response.Write("<script>alert('Message Send," + Session["seats"].ToString() + "," + Session["total"].ToString() + " ');window.location=('/payment/pay.aspx?sid=" + sid + "');</script>");
